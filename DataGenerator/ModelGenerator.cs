...
namespace DataGenerator {
	public class ModelGenerator {
		...
		private void GenerateDataTable( DataTable table ) {
			...
			AddUsings(userFile);
			//classes
			Dictionary<Section, CSClass> classes = new Dictionary<Section, CSClass>
			{
				{ Section.Model, new CSDataModel(table, Schema) },
				{ Section.Description, new CSDataDescription(table, Schema) },
				{ Section.Exemplar, new CSDataExemplaire(table, Schema) }
			};
			//classes partielles générées
			...
			if(table.Parent == null) {
				...
				classes[Section.Model].SQLTable.AddAttribute(new SQLTag());
				classes[Section.Description].SQLTable.AddAttribute(new SQLDescription());
				classes[Section.Description].SQLTable.AddAttribute(new SQLForeignKeyAttribute("model", "id"));
				classes[Section.Exemplar].SQLTable.AddAttribute(new SQLForeignKeyAttribute("model", "id"));
			} else {//Parent existe
		...
		private void GenerateStructTable( StructTable table ) {
			...
			if(table.Parent != null) {
				try {
					...
					if(cl == null) {
						ErrorManager.Error("Can't find the table " + table.Parent.Name + " within the generated tables. Notice structables can't inherits from datatables.");
					} else {
						cs.Parent = cl;
		...
		private void GenerateReferenceAttribute( CSClass cl, ValueAttribute vatt ) {
			...
			if(rt.ReferencedTable == null) {
				ErrorManager.Error("the Attribute " + cl.Name+":"+ vatt.Name + " doesn't refer to a datatable.");
				return;
			}
			CSEntity reference = GetClassByReference(rt);
			if(!(reference is CSClass)) {
				ErrorManager.Error("the Attribute " + cl.Name + ":" + vatt.Name + " doesn't refer to a datatable.");
				return;
			}
			CSClass refer = (CSClass)reference;//only cs classes : enums already done
			if(rt.ReferencedAttribute == null) {
				if(vatt.CardMax == Number.Many) {//Reference To Many
					...
					SQLForeignKeyAttribute sfk = new SQLForeignKeyAttribute(refer, vatt.Name);
				...
				if(vatt.CardMax == Number.Many) {
					...
					if(rt.ReferencedAttribute.CardMax == Number.Many) {//Many To One
						...
						SQLForeignKeyAttribute sk = new SQLForeignKeyAttribute(refer, vatt.Name);
		...
		private void CreateIndirectiontable( CSOneToManyJoint ra ) {
			CSRelationClass csRel = new CSRelationClass( ra.Class, ra.ReferencedClass, Schema, ra.Name );
		...
		private CSClass CreateJointTable( CSOneToMany ra, CSOneToMany rb ) {
			CSRelationClass csRel = new CSRelationClass(ra.Class, rb.Class, Schema, ra.Name, rb.Name);
			AddToJointure(csRel);
			AddToSql(csRel.SQLTable);
